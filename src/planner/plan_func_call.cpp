...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if(func_name == "upper")
  {
    ...
    }else throw Exception(fmt::format("func call upper not supported args more than 1"));
  }
  else if(func_name == "lower")
  {
    ...
    }else throw Exception(fmt::format("func call lower not supported args more than 1"));
  }else{};
